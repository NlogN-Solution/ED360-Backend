...
class LeaveType(Base, UUIDPKMixin, TimestampMixin, TenantMixin):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "name", name="uq_leave_types_organization_id_name"),
        Index("idx_leave_types_organization_id", "organization_id"),
    )
...
class LeaveRequest(Base, UUIDPKMixin, TimestampMixin, TenantMixin):
    ...
    # Snapshotted at creation from the org's work-day policy — deliberately
    # not recomputed later, so a subsequent policy change can't silently
    # change what an already-submitted request "costs" against the balance.
    ...
    __table_args__ = (
        ...
        Index("idx_leave_requests_organization_id", "organization_id"),
    )
