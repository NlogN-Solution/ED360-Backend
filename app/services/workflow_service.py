...
class WorkflowTemplateService:
    ...
    async def list_templates(
        ...
        if templates:
            ...
            stage_counts = dict(count_result.all())
    ...
    async def create_stage(self, template_id: UUID, data: dict[str, Any]) -> WorkflowStage:
        ...
        stage = WorkflowStage(template_id=template_id, order=(max_order or -1) + 1, **data)
    ...
    async def get_stage(self, stage_id: UUID) -> WorkflowStage | None:
        result = await self.session.execute(
            select(WorkflowStage).where(WorkflowStage.id == stage_id).options(selectinload(WorkflowStage.document_requirements))
        )
        return result.scalar_one_or_none()
    ...
    async def delete_requirement(self, requirement: WorkflowStageDocumentRequirement) -> WorkflowStageDocumentRequirement:
...
class ApplicationWorkflowService:
    ...
    async def get_step(self, step_id: UUID) -> ApplicationWorkflowStep | None:
        result = await self.session.execute(select(ApplicationWorkflowStep).where(ApplicationWorkflowStep.id == step_id))
        return result.scalar_one_or_none()
    ...
    async def update_step(
        ...
        if new_status is not None and new_status != old_status:
            ...
            if new_status == WorkflowStepStatus.CURRENT and step.started_at is None:
                step.started_at = now
            if new_status in (WorkflowStepStatus.COMPLETED, WorkflowStepStatus.FAILED, WorkflowStepStatus.SKIPPED, WorkflowStepStatus.CANCELLED):
    ...
    async def _advance_workflow(self, application_workflow_id: UUID, completed_order: int) -> None:
        ...
        next_step = next((s for s in steps if s.order > completed_order and s.status == WorkflowStepStatus.PENDING), None)
    ...
    async def add_comment(self, step: ApplicationWorkflowStep, comment: str, performed_by: UUID | None) -> WorkflowStepActivity:
    ...
    async def list_activities(self, step_id: UUID) -> list[WorkflowStepActivity]:
        result = await self.session.execute(
            select(WorkflowStepActivity).where(WorkflowStepActivity.step_id == step_id).order_by(WorkflowStepActivity.created_at.desc())
        )
...
class ChecklistService:
    ...
    async def get_item(self, item_id: UUID) -> ApplicationChecklistItem | None:
        result = await self.session.execute(select(ApplicationChecklistItem).where(ApplicationChecklistItem.id == item_id))
        return result.scalar_one_or_none()
    ...
    async def attach_uploaded_document(self, document: Document) -> list[ApplicationChecklistItem]:
        """Links a freshly uploaded document to any of the student's checklist
        items that require this document type and don't have one yet — the
        glue between the Documents tab / admin folder upload and the
        application checklist, which otherwise never hear about each other."""
        if document.document_type is None:
            return []
        result = await self.session.execute(
            select(ApplicationChecklistItem)
            .join(Application, Application.id == ApplicationChecklistItem.application_id)
            .where(
                Application.student_id == document.student_id,
                ApplicationChecklistItem.document_type == document.document_type,
    ...
    async def sync_document_review_status(self, document: Document) -> list[ApplicationChecklistItem]:
        """Mirrors a document's verify/reject decision onto any checklist item
        it's attached to, so the checklist card doesn't sit stuck on
        'Submitted' after staff have already acted on the document."""
        status_map = {
            DocumentStatus.APPROVED: ChecklistItemStatus.VERIFIED,
            DocumentStatus.REJECTED: ChecklistItemStatus.REJECTED,
        }
        new_status = status_map.get(document.status)
        if new_status is None:
            return []
        result = await self.session.execute(
            select(ApplicationChecklistItem).where(ApplicationChecklistItem.document_id == document.id)
        )
        items = list(result.scalars().all())
        for item in items:
    ...
    async def unlink_document(self, document_id: UUID) -> list[ApplicationChecklistItem]:
        """Resets checklist items pointing at a document that's about to be
        deleted, back to pending — instead of leaving them stuck on whatever
        status they had while document_id silently goes NULL underneath them
        (the FK is ON DELETE SET NULL, but that alone orphans the status)."""
        result = await self.session.execute(
            select(ApplicationChecklistItem).where(ApplicationChecklistItem.document_id == document_id)
        )
        items = list(result.scalars().all())
        for item in items:
