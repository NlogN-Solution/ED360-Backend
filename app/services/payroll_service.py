...
class PayrollService:
    ...
    async def get_salary_structure(self, user_id: UUID, organization_id: UUID | None = None) -> SalaryStructure | None:
        query = select(SalaryStructure).where(SalaryStructure.user_id == user_id)
        if organization_id is not None:
            query = query.where(SalaryStructure.organization_id == organization_id)
        result = await self.session.execute(query)
        return result.scalar_one_or_none()

    async def upsert_salary_structure(
        self, user_id: UUID, organization_id: UUID, data: dict[str, Any], changed_by: UUID | None
    ) -> SalaryStructure:
        profile = (
            await self.session.execute(
                select(EmployeeProfile).where(
                    EmployeeProfile.user_id == user_id, EmployeeProfile.organization_id == organization_id
                )
            )
        ).scalar_one_or_none()
        if profile is None:
        ...
        structure = await self.get_salary_structure(user_id, organization_id)
        previous_salary = structure.basic_salary if structure is not None else None

        if structure is None:
            structure = SalaryStructure(user_id=user_id, organization_id=organization_id, **data)
    ...
    async def get_run(self, run_id: UUID, organization_id: UUID | None = None) -> PayrollRun | None:
        ...
        if organization_id is not None:
            query = query.where(PayrollRun.organization_id == organization_id)
        result = await self.session.execute(query)
        return result.scalar_one_or_none()

    async def get_run_by_period(self, organization_id: UUID, year: int, month: int) -> PayrollRun | None:
    ...
    async def list_runs(self, organization_id: UUID, page: int, limit: int) -> tuple[list[PayrollRun], int]:
        base = select(PayrollRun).where(PayrollRun.organization_id == organization_id)
        total = (
            await self.session.scalar(
                select(func.count()).select_from(PayrollRun).where(PayrollRun.organization_id == organization_id)
            )
            or 0
        )
        query = base.order_by(PayrollRun.period_year.desc(), PayrollRun.period_month.desc()).limit(limit).offset(
            (page - 1) * limit
        )
        result = await self.session.execute(query)
    ...
    async def create_run(
        ...
        structures = list(
            (
                await self.session.execute(
                    select(SalaryStructure).where(SalaryStructure.organization_id == organization_id)
                )
            )
            .scalars()
            .all()
        ...
        for structure in structures:
            ...
            paid_leave_days = await self._count_paid_leave_days(
                structure.user_id, organization_id, period_start, period_end, work_days
            )
    ...
    async def _count_present_days(
        self, user_id: UUID, organization_id: UUID, period_start: date, period_end: date
    ) -> int:
    ...
    async def get_payslip(self, payslip_id: UUID, organization_id: UUID | None = None) -> Payslip | None:
        query = (
            select(Payslip)
        ...
        if organization_id is not None:
            query = query.where(Payslip.organization_id == organization_id)
        result = await self.session.execute(query)
        return result.scalar_one_or_none()

    async def list_payslips(self, run_id: UUID, organization_id: UUID | None = None) -> list[Payslip]:
        query = (
            select(Payslip)
            .options(selectinload(Payslip.line_items), selectinload(Payslip.payroll_run))
    ...
    async def get_employee_payslip_history(self, user_id: UUID, organization_id: UUID) -> list[Payslip]:
        query = (
            select(Payslip)
            .options(selectinload(Payslip.line_items), selectinload(Payslip.payroll_run))
            .where(Payslip.user_id == user_id, Payslip.organization_id == organization_id)
