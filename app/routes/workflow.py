...
async def get_application_workflow_service(session: AsyncSession = Depends(get_db_session)) -> ApplicationWorkflowService:
...
async def _assert_application_access(
    ...
    """Staff (admin/super_admin/counsellor) may access any application *in
    their own organization* — platform admins bypass this entirely. A student
    may only access the workflow/checklist of their own application."""
    application = await application_service.get_application(application_id)
    if application is None:
        raise NotFoundException("Application not found")
    if user.is_platform_admin:
        return
    if user.role in ("admin", "super_admin", "counsellor") and application.organization_id == user.organization_id:
        return
...
@router.get("/workflow-templates/{template_id}", response_model=WorkflowTemplateDetailRead, summary="Get workflow template")
...
@router.patch("/workflow-templates/{template_id}", response_model=WorkflowTemplateRead, summary="Update workflow template")
...
@router.delete("/workflow-templates/{template_id}", response_model=WorkflowTemplateRead, summary="Delete workflow template")
...
@router.post("/workflow-templates/{template_id}/duplicate", response_model=WorkflowTemplateDetailRead, summary="Duplicate workflow template")
...
@router.post("/workflow-templates/{template_id}/stages/reorder", response_model=list[WorkflowStageRead], summary="Reorder workflow stages")
...
@router.patch("/workflow-templates/{template_id}/stages/{stage_id}", response_model=WorkflowStageRead, summary="Update workflow stage")
async def update_workflow_stage(
    ...
    return await service.update_stage(stage, payload.model_dump(exclude_unset=True))


@router.delete("/workflow-templates/{template_id}/stages/{stage_id}", response_model=WorkflowStageRead, summary="Delete workflow stage")
async def delete_workflow_stage(
    ...
    if stage is None or stage.template_id != template_id:
        raise HTTPException(status_code=404, detail="Workflow stage not found")
    return await service.delete_stage(stage)
...
async def create_stage_requirement(
    ...
    if stage is None or stage.template_id != template_id:
        raise HTTPException(status_code=404, detail="Workflow stage not found")
    return await service.create_requirement(stage_id, payload.model_dump())
...
async def update_stage_requirement(
    ...
    return await service.update_requirement(requirement, payload.model_dump(exclude_unset=True))
...
async def delete_stage_requirement(
    ...
    if requirement is None or requirement.stage_id != stage_id:
        raise HTTPException(status_code=404, detail="Document requirement not found")
    return await service.delete_requirement(requirement)
...
@router.get("/applications/{application_id}/workflow", response_model=ApplicationWorkflowRead, summary="Get application workflow")
async def get_application_workflow(
    ...
    if workflow is None:
        raise HTTPException(status_code=404, detail="This application has no workflow yet")
    return workflow  # type: ignore[return-value]


@router.post("/applications/{application_id}/workflow", response_model=ApplicationWorkflowRead, summary="Start application workflow")
...
@router.post(
    ...
    summary="Add an ad-hoc checklist item",
)
async def create_application_checklist_item(
    ...
    application = await application_service.get_application(application_id)
    document_name = payload.custom_label or (payload.document_type.value if payload.document_type else "a document")
    program_result = await application_service.session.execute(
        select(Program.name).where(Program.id == application.program_id)
    )
    program_name = program_result.scalar_one_or_none() or ""
    ...
    subject, body = await template_service.render(
        ...
        {"document_name": document_name, "program_name": program_name},
    )
...
async def update_application_checklist_item(
    ...
    return await service.update_item(item, payload.model_dump(exclude_unset=True))  # type: ignore[return-value]
